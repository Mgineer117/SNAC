...
class OP_Controller(BasePolicy):
    ...
    def scale_reward(self, rew):
        """
        The reward generated by the option is too small so that
            - there exists numerical issue to optimize
            - not compatible pre-defined PPO training param
            - just arbitraily chosen min/max are used.
        """
        rew = (rew - self.rew_min) / (self.rew_max - self.rew_min)
        return rew
    ...
    def learn(self, batch, z):
        ...
        # print(
        #     torch.max(torch.abs(rewards)).clone().detach().cpu().numpy(),
        # )
        ...
        # print()
        # print(
        #     torch.max(torch.abs(values)).clone().detach().cpu().numpy(),
        #     torch.max(torch.abs(returns)).clone().detach().cpu().numpy(),
        # )
